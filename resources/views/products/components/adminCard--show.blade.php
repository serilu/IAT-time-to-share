<article class="productCard a-popup">
        ...
        <p class="productCard__message">Dit product is uitgeleend aan {{$users[$products->lener-1]->name}} tot {{  gmdate("Y-m-d\ H:i:s",strtotime($products->updated_at) + $products->uitleenTijd) }}</p>
            <a class="productCard__back__to__home__own" href="/admin"> Back to home</a>
        ...
        <section class="productCard__btnSection">
            <form enctype="multipart/form-data" action="/admin/{{ $products->id }}" method="POST">
            @csrf
            @method("PUT")

            <button type="submit" name="lening" class="ProductCard__verwijder">Verwijderen</button>
            
            </form>
            
            <a class="productCard__back__to__home" href="/admin"> Back to home</a>
        </section>
        @endif
    </article>